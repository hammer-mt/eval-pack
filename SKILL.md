---
name: eval-pack
description: Build an evaluation pack — a single self-contained HTML slide deck (left-hand index navigation) with diagrams, screenshots, animated GIFs, and real captured test output that proves completed work actually runs, so a human can review an hour of agent work in a few minutes. Use whenever the user says "eval pack", "evaluation pack", "review pack", "make this reviewable", "show me what you did", "prove it works", or asks how to review or verify a completed piece of work — and offer one proactively after finishing any substantial feature, refactor, bugfix, or multi-file change.
---
...
3. **Map** — a diagram of what changed: boxes and arrows over real file and function names showing how the pieces connect or how data flows. Draw it with the **`architecture-diagram` skill** (invoke it via the Skill tool, or read `~/.claude/skills/architecture-diagram/SKILL.md`): follow its design system — semantic component colors, spacing rules, arrow z-order, legend placement — but embed **only the inline `<svg>`** in the slide. Do not include its HTML shell, Google Fonts link, export toolbar, or CDN scripts (html2canvas/jsPDF); those violate the pack's offline single-file rule. Use a generic `monospace` font stack in place of JetBrains Mono, and keep the diagram's dark background rect so it reads as a self-contained panel on both deck themes — the template's example Map slide shows the pattern. Simple and accurate beats elaborate.
4. **One slide per work item** — open with one plain-English sentence saying what changed in behavior terms and why it matters; then key `file:line` references, and the evidence right on the slide next to the claim it proves.
5. **Test artifacts** — the suite output; for scenario-style testing, one evidence block (screenshot, GIF, or transcript) per scenario.
6. **Limitations & caveats** — what you didn't verify, judgment calls you made, known gaps, anything the reviewer should double-check by hand. Never skip this slide or leave it empty; there is always something, and naming it is what makes the rest of the pack credible.
...
- **Diagrams are inline SVG** with real names from the codebase, not generic placeholders. Style them per the `architecture-diagram` skill's design system (SVG only — no external fonts or scripts from that skill).
